...
use jj_lib::read_only_user_repo::ReadonlyUserRepo;
...
",
            ...
            .await
            .map_err(|_e| user_error("TODO: incomplete error handling"))?;
        writeln!(
            ui.status(),
    ...
    async fn update_working_copy(
        ...
        let stats = self
            ...
            .await
            .map_err(|e| internal_error(e))?; // TODO: This is incorrect.
    ...
    pub fn start_transaction(&mut self) -> WorkspaceCommandTransaction<'_> {
        let inner = self
            .operation_runner
            .start_transaction(self.workspace_name(), self.env.command.string_args());
    ...
    async fn finish_transaction(
        ...
        // TODO: find some way to restore this warning.
        // Err(UserError { error, .. }) => {
        //     writeln!(
        //         ui.warning_default(),
        ...
        // TODO: find a way to restore this warning.
...
async fn handle_stale_working_copy(
    ...
    match workspace_operation_runner::handle_stale_working_copy(locked_wc, repo, workspace_name)
        .await
    {
        Ok(Some((repo, commit))) => Ok(Some((repo, commit))),
        Ok(None) => Ok(None),
        Err(
            e @ HandleStaleWorkingCopyError::Backend(_) | HandleStaleWorkingCopyError::RepoLoad(_),
