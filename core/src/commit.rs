...
//! Contains the [`Commit`] type which is a soft wrapper over a [`backend::Commit`] with additional
//! methods and helpers.
...
/// A `Commit` is thin wrapper over a [`backend::Commit`] with a reference to its
/// [`CommitId`] and the [`Store`] it came from.
