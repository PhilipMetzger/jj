...
//! The [`Index`] provides ways to create an index over a [`Backend`]'s commits.
...
/// A readonly View over an [`Index`].
pub trait ReadonlyIndex: Any + Send + Sync {
    /// Convert this ReadonlyIndex to the base [`Index`].
    fn as_index(&self) -> &dyn Index;

    /// Get a view over the [`ChangeIdIndex`] for the given `heads`.
    fn change_id_index(&self, heads: &mut dyn Iterator<Item = &CommitId>)
    -> Box<dyn ChangeIdIndex>;

    /// Start a modification on this ReadonlyIndex converting it into an [`MutableIndex`].
...
/// A [`MutableIndex`] is a trait which you can implement to build up a larger [`Index`]. The
/// mechanism used is similar to a [log structured merge tree], you build up individual mutable
/// Indexes which get merged to a larger one after some time.
///
/// [log structured merge tree]: https://en.wikipedia.org/wiki/Log-structured_merge-tree  
#[async_trait]
pub trait MutableIndex: Any {
    /// Convert this MutableIndex to its base [`Index`] type.
    fn as_index(&self) -> &dyn Index;

    /// Get a view on the [`ChangeIndex`] for the given `heads`].
    fn change_id_index(
        &self,
        heads: &mut dyn Iterator<Item = &CommitId>,
    ) -> Box<dyn ChangeIdIndex + '_>;

    /// Adds a `commit` to the given Index.
    async fn add_commit(&mut self, commit: &Commit) -> IndexResult<()>;

    /// Merge this Index with another [`ReadonlyIndex`], updating this with `others` commits.
