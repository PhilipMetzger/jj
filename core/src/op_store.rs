...
//! Contains the [`OpStore`] trait and the related [`Operation`] and [`OperationMetadata`] types.
...
/// A specialized [`Result`] type for OpStore errors.
pub type OpStoreResult<T> = Result<T, OpStoreError>;

/// An Opstore creates new [`Operations`] and [`Views`] which represent jj-internal data at a current snapshot or
/// time.
#[async_trait]
pub trait OpStore: Any + Send + Sync + Debug {
    /// Get the name of this [`OpStore`].
    fn name(&self) -> &str;

    /// Get the ID of the root operation.
    fn root_operation_id(&self) -> &OperationId;

    /// Get access to a [`View`] at
    async fn read_view(&self, id: &ViewId) -> OpStoreResult<View>;

    /// Write a new [`View`] to the Store.
    async fn write_view(&self, contents: &View) -> OpStoreResult<ViewId>;

    /// Read the state of the Store at the given `id` and get the Operation.
    async fn read_operation(&self, id: &OperationId) -> OpStoreResult<Operation>;

    /// Write a new [`Operation`] to the Store.
