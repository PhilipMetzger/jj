...
//! Contains the [`Repo`] trait which is needed to inspect a Repository. It also contains a base
//! implementation of [`ReadonlyRepo`] without understanding any concrete implementation.
...
/// A [`Repo`] contains accessors to all the different Backends such as the [`Index`] and provides
/// a simple way to get a [`Store`].
pub trait Repo {
    ...
    /// Find the shortest ChangeId prefix length for the given `target_id_bytes`.
