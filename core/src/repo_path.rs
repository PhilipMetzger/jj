...
//! A [`RepoPath`] is a safe abstraction over different [`Path`] implementations across operating
//! systems. It provides a safe way to interact with paths in a jj repo.
...
mod tests {
 ...
 wnynunwo 2de79589 "core: Lower `Backend`, `Index` and their dependencies from `jj-lib` into it" (rebased revision)
...
//! Contains jj-specific Path types to safely deal with them across operation systems
//! and to prevent certain path oriented attacks.
