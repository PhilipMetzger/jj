...
//! A [`Store`] provides a caching layer over a [`Backend`] and its [`backend::Commit`]'s and
//! [`backend::Tree`]'s, it also has some utility functions for interacting with a [`Signer`].
...
impl Store {
    pub fn new(backend: Box<dyn Backend>, signer: Signer, merge_options: MergeOptions) -> Self {
        Self {
            backend,
            signer,
    ...
    pub async fn get_backend_commit(&self, id: &CommitId) -> BackendResult<Arc<backend::Commit>> {
        {
