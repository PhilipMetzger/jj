...
//! Contains `Tree` a shim similar to [`Commit`] over both a [`backend::TreeId`] and
//! a [`backend::Tree`]. It also has helpers to make the creation of it easier.
...
/// A Tree stores information about a directory and from which `Store` it comes from.
#[derive(Clone)]
pub struct Tree {
