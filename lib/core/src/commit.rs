...
//! Contains the [`Commit`] type which is a soft wrapper over a
//! [`backend::Commit`] with additional methods and helpers.
...
/// A `Commit` is thin wrapper over a [`backend::Commit`] with a reference to
/// its [`CommitId`] and the [`Store`] it came from.
#[derive(Clone, serde::Serialize)]
pub struct Commit {
    /// The `Store` this commit is associated with.
    #[serde(skip)]
    store: Arc<Store>,
    #[serde(rename = "commit_id")]
    /// The Id of the inner commit.
    id: CommitId,
    /// The actual backend commit this encompasses.
    #[serde(flatten)]
