...
pub trait ReadonlyIndex: Any + Send + Sync {
    ...
    /// Start a modification on this ReadonlyIndex converting it into an
    /// [`MutableIndex`].
...
/// A [`MutableIndex`] is a trait which you can implement to build up a larger
/// [`Index`]. The mechanism used is similar to a [log structured merge tree],
/// you build up individual mutable Indexes which get merged to a larger one
/// after some time.
///
...
pub trait MutableIndex: Any {
    ...
    /// Merge this Index with another [`ReadonlyIndex`], updating this with
    /// `others` commits.
