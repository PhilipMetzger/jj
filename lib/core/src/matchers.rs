...
/// `Matcher`'s are used to specify how the snapshotting path traverses
...
/// Represents a way to match on files or directories.
...
/// Matches directories and files on the given prefix.
#[derive(Debug)]
...
/// Describes the way you can use the `PrefixMatcher`.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
enum PrefixNodeKind {
    /// Represents an intermediate directory.
    #[default]
...
impl<M1: Matcher, M2: Matcher> UnionMatcher<M1, M2> {
    /// Create a new `UnionMatcher` from the given inputs.
...
impl<M1: Matcher, M2: Matcher> DifferenceMatcher<M1, M2> {
    /// Create a new `DifferenceMatcher` with the given `wanted` and `unwanted`
    /// submatchers.
...
impl<M1: Matcher, M2: Matcher> IntersectionMatcher<M1, M2> {
    /// Create a new `IntersectionMatcher` by matching on both inputs.
    pub fn new(input1: M1, input2: M2) -> Self {
        Self { input1, input2 }
    }
}
