...
//! Utilities for binary Objects, such as a macro for a [`ObjectId`] newtype.
//! See [`id_type!`].
...
/// Describes the common functionality of a binary ID type.
pub trait ObjectId {
    /// Get actual Name of underlying type.
    // this strips the trailing `Id`.
    fn object_type(&self) -> String;

    /// Get access to the underlying bytes.
    fn as_bytes(&self) -> &[u8];

    /// Get access to another copy of the underlying bytes.
    fn to_bytes(&self) -> Vec<u8>;

    /// Convert the underlying bytes into a hex-formatted string.
    fn hex(&self) -> String;
}
...
/// Defines the required methods needed for a [`id_type`] which is a newtype
/// over a `Vec<u8>`.
#[macro_export]
...
impl HexPrefix {
    ...
    /// Create a new `HexPrefix` from the given bytes.
    pub fn from_bytes(bytes: &[u8]) -> Self {
        Self {
...
pub enum PrefixResolution<T> {
    /// Nothing matched.
    NoMatch,
    /// There was a single match, containing `T`.
    SingleMatch(T),
    /// There were multiple matches and thus ambiguous.
    AmbiguousMatch,
}

impl<T> PrefixResolution<T> {
    /// Apply `f` on this `PrefixResolution` returning the new result.
    ...
    /// Apply `f` on this `PrefixResolution` returning the new result if there's
    /// a single match.
...
impl<T: Clone> PrefixResolution<T> {
    /// Combine the results of two different `PrefixResolution`'s.
