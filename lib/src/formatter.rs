...
impl<T: Formatter, H> HeadingLabeledWriter<T, H> {
    /// Create a new `HeadingLabeledWritter` for the given `formatter` and
    /// `heading`.
...
impl<T: Formatter, H: fmt::Display> HeadingLabeledWriter<T, H> {
    /// Write the given `args` to the underlying with the given heading if one
    /// exists.
...
impl FormatRecorder {
    /// Create a new `FormatRecorder` with `maybe_color` indicating that it
    ...
    /// Replay this FormatRecorder on the `formatter` with the given
    /// `write_data` function.
    pub fn replay_with(
        &self,
