...
pub enum UserRevsetEvaluationError {
    ...
    /// The revset failed to evaluate because there was a syntactical error or otherwise.
    #[error(transparent)]
...
pub struct RevsetExpressionEvaluator<'repo> {
    ...
    repo: &'repo dyn Repo,
    /// The registered extensions with this Evaluator, usually provided by the environment.
...
impl<'repo> RevsetExpressionEvaluator<'repo> {
    /// Create a new `RevsetExpressionEvaluator` for the given `repo` and `expression`.
    /// If you provide `extensions` they also will be respected during the evaluation.
    pub fn new(
        repo: &'repo dyn Repo,
        extensions: Arc<RevsetExtensions>,
        id_prefix_context: &'repo IdPrefixContext,
...
/// Warn if the `config` contains a user-provided name which matches the built-in expressions. All
/// output is written to `warn`.
