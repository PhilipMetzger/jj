...
//! Contains a low-level `UserError` on which good diagnostics such as the CLI can be built on.
use std::error;
use std::io;
use std::iter;
use std::sync::Arc;
...
pub enum UserErrorKind {
    ...
    Cli,
    /// An error occured when reading or writing to a pipe, e.g by calling Git in a subprocess.
    BrokenPipe,
    /// An internal error occured, inspect the error for more information.
    Internal,
}

/// A `UserError` describes the kind of an error and its source and optionally provides hints on
/// how to resolve it.
#[derive(Clone, Debug)]
pub struct UserError {
    /// The kind describes the source of the error.
    pub kind: UserErrorKind,
    /// The inner error containing the actual `Error`, is an `Arc` for threadsafe evaluations.
    pub error: Arc<dyn error::Error + Send + Sync>,
    /// The additional hints this error has generated, usually printed to the CLI.
...
impl UserError {
    ...
    /// Create a new `UserError` with the given `kind` and `message` from `source`.
    pub fn with_message(
        kind: UserErrorKind,
...
pub enum ErrorHint {
    ...
    PlainText(String),
    /// The hint is formatted and contains ANSI-Escape codes which need to printed to the terminal.
...
/// Create a new `UserError` from an error stemming from a user input with the given message.
...
/// Create a new `UserError` with the source being the config (`config.toml` or otherwise).
pub fn config_error(err: impl Into<Box<dyn error::Error + Send + Sync>>) -> UserError {
    UserError::new(UserErrorKind::Config, err)
}

/// Create a new `UserError` with the source being the config (`config.toml` or otherwise) and
/// `message`.
...
/// Create a new `UserError` where the source is an internal error with the given `message`.
...
/// Provide useful URLs for revset parse errors for `err` in the case the user makes syntax errors.
...
/// Provide helpful messages for `err` for ambiguous commmits in during the revset resolution.
