...
pub struct WorkspaceEnvironment {
    ...
    /// extensions such `is_lgtm()` or `green(<id>)`.
...
impl WorkspaceEnvironment {
    /// Create a new `WorkspaceEnvironment` for the given `Workspace` in the
    ...
    /// Load the `revsets.immutable_heads()` expression, returns an error if it
    ...
    /// Load the `revsets.short-prefixes` expression from the settings, falling
...
/// Load any existing fileset aliases from the `fileset-aliases` key in the
/// config.
...
/// Load any existing revset aliases from the `revset-aliases` key in the
/// config.
