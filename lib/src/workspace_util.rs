...
pub struct WorkspaceEnvironment {
    ...
    /// The Revset extensions which this environment also carries, this is usually relevant for
    /// downstreams such as Google which have custom extensions such `is_lgtm()` or `green(<id>)`.
    revset_extensions: Arc<RevsetExtensions>,
    /// The default ignored remote TODO: incomplete description .
    ...
    /// The `RepoPathUiConverter` which is used to print paths for warnings and more.
    // TODO: make private
    pub path_converter: RepoPathUiConverter,
    /// The name of the `Workspace` this environment is assocaited with.
    // TODO: make private
    pub workspace_name: WorkspaceNameBuf,
    /// The `immutable_heads()` revset expression read from the `settings` / `config.toml`.
    immutable_heads_expression: Arc<UserRevsetExpression>,
    /// The `revsets.short-prefixes` revset expression read from the `settings` / `config.toml`.
...
impl WorkspaceEnvironment {
    /// Create a new `WorkspaceEnvironment` for the given `Workspace` in the given current working
    /// directory `cwd`. If `revset_extensions` are set the environment will also respect them.
    /// `warn` will be used by the environment to print warnings in jj's own CLI this will be
    ...
    /// Get a reference to the associated `RepoPathUiConverter`.
    pub fn path_converter(&self) -> &RepoPathUiConverter {
        &self.path_converter
    }

    /// Get a reference to associated `WorkspaceName`.
    pub fn workspace_name(&self) -> &WorkspaceName {
        &self.workspace_name
    }

    /// Get a mutable reference to the associated `RevsetAliasesMap`.
    pub fn revset_aliases_map(&mut self) -> &mut RevsetAliasesMap {
        &mut self.revset_aliases_map
    }

    /// Get a reference to the associated `RevsetExtensions`.
    ...
    /// Load the `revsets.immutable_heads()` expression, returns an error if it invalid or not
    /// found.
    ...
    /// Load the `revsets.short-prefixes` expression from the settings, falling back to
    /// `revsets.log` if it doesn't exist. All diagnostics will be added to `diagnostics`.
    ...
    /// Returns first immutable commit for the given `repo`. If `ignore_immutable` is set immutable
    /// revisions will be ignored and the calculation starts from the root expression.
...
/// Load any existing fileset aliases from the `fileset-aliases` key in the config.
...
/// Load any existing revset aliases from the `revset-aliases` key in the config.
